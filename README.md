...
- **隔離修復工具** — 管理員可在設定頁掃描並安全修復 legacy folder ownership 問題
- **更完整搜尋** — 可搜尋日記、對話、檔案，以及日記照片的 AI 描述
- **對話可取消** — 送出中的對話可在前端取消，server 會停止後續流程並清理半途資料
- **版本更新提示** — 使用者升級後第一次開啟會看到「在你不在的時候，我們加入了這些功能」
- **可複製格式回覆** — 規劃、待辦與範本內容可用 checklist 與 code block 呈現，方便直接複製貼上
- **待辦進度卡** — 對話中的 task list 會顯示完成數，完成後可自動收合成摘要
- **互動式待辦同步** — 可直接在對話中勾選待辦，UI、完成態與可複製 markdown 會同步更新
- **AI 先判斷對話需求** — 對話會先用 AI 讀取目前訊息、標題與最近歷史，再決定要規劃、直接解題、行動支持或陪伴反思
- **規劃與行動支持** — 規劃型問題偏具體輸出；情緒低潮但有具體問題時，會先穩住情緒再拆解下一步
- **檔案管理** — 支援圖片、PDF 附件上傳
- **使用者系統** — JWT 認證、多使用者管理
