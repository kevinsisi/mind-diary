...
export async function dismissReleaseNotes(page: Page): Promise<void> {
  ...
  if (await button.waitFor({ state: 'visible', timeout: 3000 }).then(() => true).catch(() => false)) {
    await button.click();
  }
}

export async function waitForAssistantReply(page: Page, text?: RegExp): Promise<void> {
  const assistantMessages = page.locator('[data-message-role="assistant"], .prose, .whitespace-pre-wrap');
  if (text) {
...
export async function cleanupGuestSessions(request: Request, marker: string): Promise<void> {
  ...
  const body = (await sessionsResponse.json()) as { sessions?: Array<{ id: number; title: string; last_message?: string }> };
