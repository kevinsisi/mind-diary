...
- 如果內容涉及行程規劃、待辦事項、步驟整理或適合讓使用者直接複製貼上，優先用 markdown checklist 或 fenced code block 整理重點`;
  ...
  let fullText = await callGeminiText(chatSystemPrompt, prompt, 1000, {
    maxRetries: 5,
    callType: 'chat-agent',
...
const SUPPORT_ACTION_CHAT_PROMPT = `你是情緒急救與問題拆解助理。多位 AI 好友已經提供觀點，但你現在要直接幫使用者穩住情緒並處理眼前問題。

規則：
- 繁體中文
- 不要用夥伴人格格式輸出
- 先用 1 句話接住情緒，接著立刻轉成具體問題拆解
- 不要只有「你辛苦了」「慢慢來」「我懂你」這類空泛安慰
- 使用者遇到問題時，必須提供可立即採取的下一步、可說出口的句子或低負擔待辦
- 缺資料時可以做合理假設，但不要用反問取代幫助；最多問 1 個真正必要的問題
- 如果內容出現自傷、想死、不想活、傷害自己或傷害他人的風險，優先給安全步驟：離開危險物品、聯絡可信任的人、立即撥打當地緊急電話
- 內容務實、簡潔、有條理
- 先把多位夥伴的內容去重，再整合成一份不重複的行動支持

輸出格式：
## 我先幫你穩住
<1 句接住情緒，並指出現在最需要先處理什麼>
...
async function synthesizeChat(
  ...
  const systemPrompt = isConciseReply
    ? buildNicknameInstruction(nickname || '') +
      '你是心靈日記的最終回答整理器。請根據使用者問題與多位好友的觀點，輸出單一最終答案。規則：繁體中文、嚴格遵守使用者格式要求、只輸出答案本身。'
    : buildNicknameInstruction(nickname || '') + MASTER_CHAT_PROMPT;

  const fullText = await callGeminiText(systemPrompt, prompt, 4096, {
    maxRetries: 5,
    callType: 'chat-master',
    disableThinking: true,
    timeoutMs: 30000,
  });
...
async function synthesizePlanningChat(
  ...
  const systemPrompt = isConciseReply
    ? buildNicknameInstruction(nickname || '') +
      '你是規劃需求的最終回答整理器。請根據使用者需求與多位好友觀點，輸出一個最終答案。規則：繁體中文、嚴格遵守使用者格式要求、只輸出答案本身。'
...
async function synthesizeSupportActionChat(
  agentResults: Array<{ agentId: string; result: string }>,
  userMessage: string,
  contextStr: string,
  memoryStr: string,
  ...
  onEvent({ type: 'synthesizing', message: '🧩 穩住情緒並拆解下一步...' });
  ...
  let prompt = `使用者現在心情很糟，而且有問題需要處理：${userMessage}\n\n`;
  if (imagePart) prompt += `【使用者同時上傳了圖片（輔助資訊）】\n${imagePart}\n\n`;
  if (memoryStr) prompt += `【使用者跨對話記憶（僅供參考）】\n${memoryStr}\n\n`;
  if (contextStr) prompt += `【相關資料】\n${contextStr}\n\n`;
  if (historyStr) prompt += `【最近對話紀錄】\n${historyStr}\n\n`;
  prompt += `以下是各位好友的觀點，請只取有幫助的部分並去重：\n\n${analysisBlock}`;
  ...
  const systemPrompt = isConciseReply
    ? buildNicknameInstruction(nickname || '') +
      '你是情緒困境的最終回答整理器。請根據使用者問題與多位好友觀點，輸出一個最終答案。規則：繁體中文、嚴格遵守使用者格式要求、只輸出答案本身。'
    : buildNicknameInstruction(nickname || '') + SUPPORT_ACTION_CHAT_PROMPT;

  const fullText = await callGeminiText(systemPrompt, prompt, 2200, {
    maxRetries: 5,
    callType: 'chat-support-action-master',
    disableThinking: true,
    timeoutMs: 30000,
  });
  onEvent({ type: 'synthesizing', content: fullText });
...
async function synthesizePracticalAnswerChat(
  ...
  const systemPrompt = isConciseReply
    ? buildNicknameInstruction(nickname || '') +
      '你是實用問題的最終回答整理器。請根據使用者問題與多位好友觀點，直接輸出答案本身。規則：繁體中文、嚴格遵守格式要求、不要輸出角色名、emoji 或多段陪聊。'
    : buildNicknameInstruction(nickname || '') +
      '你是實用解題助理。你的任務是直接給出可用、可執行、具體的答案。規則：繁體中文、答案優先、第一句就給主結論、不要多角色格式、不要情緒陪聊蓋過解題、不要用空泛套話。';
...
async function synthesizePracticalFallbackDirect(
  ...
  if (conciseInstruction) {
    ...
    prompt +=
      '請直接給可用答案，不要陪聊，不要角色格式，不要先反問。若資訊不足，可給一個主推薦和 1-2 個備選。';
  }

  const systemPrompt = conciseInstruction
    ? buildNicknameInstruction(nickname || '') +
      '你是實用問題的直接回答保底助手。請直接輸出答案本身，嚴格遵守格式要求，不要角色名與 emoji。'
    : buildNicknameInstruction(nickname || '') +
      '你是實用解題助理的保底模式。當多代理分析不可用時，你必須直接給使用者可執行答案。規則：繁體中文、答案優先、不要角色格式、不要情緒陪聊。';
...
async function synthesizeDirectiveAdviceDirect(
  ...
  const hasConcreteTopic =
    /主管|工作|感情|前任|失戀|焦慮|壓力|睡不著|晚餐|午餐|早餐|宵夜|吃|餐廳|去哪|散心|朋友|家庭|家人|同事|職涯|學習|選哪個|選一個/i.test(
      recent,
    );
  if (!hasConcreteTopic) {
    const clarification =
      '可以，直接告訴我你現在要處理的主題，例如工作、人際、感情、吃飯或行程，我就直接給建議。';
  ...
  if (historyStr) prompt += `【最近對話紀錄】\n${historyStr}\n\n`;
  prompt +=
    '請直接給 2-4 條可執行建議或一個最優先的下一步。不要安慰、不要角色格式、不要追問、不要情緒探索蓋過解題。';
...
function allAgentResultsUnavailable(
  agentResults: Array<{ agentId: string; result: string }>,
): boolean {
...
function buildPracticalEmergencyResponse(userMessage: string): string {
  ...
  if (/午餐|晚餐|宵夜|吃什麼|吃甚麼|吃啥|餐廳|美食/.test(text))
    return '先選一間離你最近、評價穩定、你平常就會吃的店，不要再耗在選擇上。';
  if (/去哪|去哪裡|散心|週末/.test(text))
    return '先去一個移動成本低、能立刻出發的地方，例如附近公園、河堤、商場或咖啡店，重點是先出門。';
  if (/主管|溝通|談/.test(text))
    return '先整理你的目標、事實和希望主管回應的具體內容，再約一個不被打擾的時間直接談。';
...
function buildSupportActionEmergencyResponse(userMessage: string, historyStr: string): string {
  const recent = `${historyStr}\n${userMessage}`;
  if (/想死|自殺|不想活|傷害自己|自殘|輕生|傷害別人/.test(recent)) {
    return '先把自己和危險物品拉開，立刻聯絡身邊可信任的人陪你；如果你可能傷害自己或別人，現在就打 119、110 或當地緊急電話。';
  }
  if (/主管|同事|客戶|工作|公司|被罵|被羞辱/.test(recent)) {
    return '先做三件事：一，離開現場或暫停回覆十分鐘；二，把剛剛發生的事實寫下來，不先評價自己；三，整理一句你要對對方說的界線或需求。';
  }
  if (/失戀|分手|前任|伴侶|男友|女友|吵架/.test(recent)) {
    return '先不要逼自己立刻想通。現在先吃點東西或洗澡，接著把最想傳給對方的話寫在備忘錄，不要立刻送出，晚一點再決定。';
  }
  if (/焦慮|壓力|睡不著|心悸|胸悶|喘不過氣/.test(recent)) {
    return '先把問題縮小：喝水、坐下慢慢吐氣三次，然後只寫「現在最擔心的三件事」和「今天能做的一件小事」。先做那一件就好。';
  }
  return '先不要逼自己一次解決全部。現在先寫下最卡的三件事，只挑一件今天能處理的，做出第一個十分鐘內能完成的小動作。';
}
...
function isPlanningIntent(...inputs: Array<string | undefined>): boolean {
  ...
  return /旅行|旅遊|行程|規劃|安排|韓國|日本|首爾|釜山|濟州|大阪|東京|京都|福岡|沖繩|出國|自由行|待辦|清單|計畫|沒有頭緒|沒頭緒/i.test(
    combined,
  );
}

function isSupportActionIntent(
  currentMessage: string,
  historyStr?: string,
  sessionTitle?: string,
): boolean {
  const recentUserHistory = String(historyStr || '')
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter((line) => line.startsWith('使用者：'))
    .slice(-4)
    .join('\n');
  const combined = [sessionTitle, recentUserHistory, currentMessage].filter(Boolean).join('\n');
  const current = String(currentMessage || '');
  const wantsOnlyListening = /只想被聽|只想抱怨|先不要給建議|不要給建議|陪我一下|聽我說/i.test(
    current,
  );
  if (wantsOnlyListening) return false;

  const distress =
    /心情.{0,4}(糟|差|爛|不好)|很糟|很差|很煩|煩死|崩潰|快崩潰|難過|焦慮|壓力|委屈|低落|失落|痛苦|快受不了|受不了|撐不住|沒力|好累|慌|不知道怎麼辦|怎麼辦|救命|卡住/i.test(
      combined,
    );
  const problem =
...
function extractLastAssistantMessage(historyStr?: string): string {
  ...
  for (let i = lines.length - 1; i >= 0; i -= 1) {
    if (lines[i].startsWith('助理：') || lines[i].startsWith('助手：')) {
      return lines[i].replace(/^(助理|助手)：/, '').trim();
    }
  }
...
function hasPracticalRefinementContext(historyStr?: string): boolean {
  ...
  const practicalTopic =
    /晚餐|午餐|早餐|宵夜|吃什麼|吃甚麼|吃啥|餐廳|美食|哪家|哪裡吃|吃哪間|吃哪家|晚點吃什麼|推薦我去哪|去哪裡散心|去哪散心|週末去哪|週末推薦我去哪|.+跟.+選一個|.+還是.+選一個|.+跟.+選哪個|.+或.+選哪個|.+還是.+選哪個|怎麼跟.+溝通|怎麼和.+溝通|如何跟.+溝通|如何和.+溝通|怎麼跟.+談|怎麼和.+談|如何跟.+談|如何和.+談|.+怎麼談比較好|推薦我怎麼|建議我怎麼|直接告訴我怎麼|教我怎麼/i;
  return (
    recentUsers.some(
      (message) => practicalTopic.test(message) || isPracticalRefinementMessage(message),
    ) && Boolean(lastAssistant)
  );
}

function buildPracticalRefinementDeterministic(
  currentMessage: string,
  historyStr: string,
): string | null {
  const current = String(currentMessage || '');
  ...
  if (isFood && isTaipeiStation) {
    if (/預算低一點|便宜一點|不要太花錢|選最便宜的/.test(current)) {
      if (/南陽街|補習街/.test(lastAssistant))
        return '南陽街或補習街的小吃店。價格通常更低，也比商場餐廳划算。';
      return null;
    }
    if (/不要排隊/.test(current)) {
      if (/台北地下街|微風台北車站|微風廣場/.test(lastAssistant))
        return '台北地下街或微風台北車站美食街。翻桌快、選擇多，也比較不容易卡在排隊。';
      return null;
    }
    if (/近一點|近一些|近一點就好|選近一點的/.test(current)) {
      if (/台北地下街|微風台北車站|微風廣場/.test(lastAssistant))
        return '台北地下街。它就在車站下方，移動最少，也方便快速吃完。';
      return null;
    }
  ...
  if (isPlace) {
    if (/近一點|近一些|近一點就好|選近一點的/.test(current)) {
      if (/公園|河堤|商場|咖啡店/.test(lastAssistant))
        return '附近公園或河堤。移動成本最低，也最容易立刻出門。';
      return null;
    }
    if (/不要太花錢|便宜一點|預算低一點|選最便宜的/.test(current)) {
      if (/公園|河堤|商場|咖啡店/.test(lastAssistant))
        return '附近公園、河堤或圖書館。這些地方幾乎不用花錢，也能讓你換個環境。';
      return null;
    }
    if (/附理由|給理由/.test(current)) {
      if (/公園|河堤|商場|咖啡店/.test(lastAssistant))
        return '因為你現在要的是低成本、低負擔、可以立刻開始的選項，越近、越便宜、越不用規劃的地方越適合。';
      return null;
    }
    if (/唯一答案|直接給唯一答案|直接給一個答案|只給一個/.test(current)) {
...
function isPracticalAnswerIntent(currentMessage: string, historyStr?: string): boolean {
  const current = String(currentMessage || '');
  ...
  const recommendationTopic =
    /晚餐|午餐|早餐|宵夜|吃什麼|吃甚麼|吃啥|餐廳|美食|哪家|哪裡吃|吃哪間|吃哪家|晚點吃什麼|推薦我去哪|去哪裡散心|去哪散心|週末去哪|週末推薦我去哪/i;
  const choiceTopic =
    /.+跟.+選一個|.+還是.+選一個|.+跟.+選哪個|.+或.+選哪個|.+還是.+選哪個|幫我選.+|替我選.+/i;
  const howToTopic =
    /怎麼跟.+溝通|怎麼和.+溝通|如何跟.+溝通|如何和.+溝通|怎麼跟.+談|怎麼和.+談|如何跟.+談|如何和.+談|.+怎麼談比較好|推薦我怎麼|建議我怎麼|直接告訴我怎麼|教我怎麼/i;
  const practicalTopic = new RegExp(
    `${recommendationTopic.source}|${choiceTopic.source}|${howToTopic.source}`,
    'i',
  );
  const answerPush =
    /給我答案|直接告訴我|直接回答|幫我選|替我選|選一個|就直接說|不要再問|直接說結論|直接給一個答案/i;
  const refinementPush =
    /預算低一點|便宜一點|不要排隊|近一點|近一些|近一點就好|不要太花錢|附理由|給理由|唯一答案|直接給唯一答案|直接給一個答案|只給一個|選最便宜的|選近一點的/i;
  const emotionalTopic =
    /焦慮|難過|委屈|失戀|沒胃口|心情很差|很亂|很煩|不知道怎麼辦|被罵|被羞辱|冷落/i;
  ...
  if (refinementPush.test(current)) {
    ...
    if (
      recentUserMessages.some(
        (message) =>
          practicalTopic.test(message) || answerPush.test(message) || refinementPush.test(message),
      )
    ) {
      return true;
    }
...
function isDirectiveAdviceIntent(currentMessage: string, historyStr?: string): boolean {
  ...
  const directAdvice =
    /不要安慰我|直接給建議|給我建議|直接給答案|直接給方法|直接說怎麼做|不要陪聊/i;
  ...
  if (
    genericAnswerPush.test(current) &&
    emotionalContext.test(recent) &&
    !isPracticalAnswerIntent(current, historyStr)
  )
    return true;
  return false;
}
...
function extractTravelDestination(userMessage: string): string {
  const match = userMessage.match(
    /韓國|日本|首爾|釜山|濟州|大阪|東京|京都|福岡|沖繩|巴黎|倫敦|曼谷|新加坡/,
  );
  return match?.[0] || '這趟旅行';
}
...
function getSupportActionSelections(content: string) {
  const safetyRisk = /想死|自殺|不想活|傷害自己|傷害別人|自殘|輕生/i.test(content);
  const boundaryProblem =
    /被罵|被羞辱|委屈|不公平|界線|主管|同事|客戶|朋友|家人|伴侶|男友|女友|吵架|衝突/i.test(content);
  const healthProblem = /睡不著|失眠|心悸|胸悶|喘不過氣|身體|生病|沒胃口|吃不下|頭痛|胃痛/i.test(
    content,
  );
  const riskProblem = /害怕|擔心|不安|恐慌|出事|事故|風險|危險/i.test(content);

  const third =
    safetyRisk || healthProblem
      ? { agent: AGENTS.dran, reason: '補上身心安全與必要求助界線，避免只用情緒安慰帶過。' }
      : boundaryProblem
        ? { agent: AGENTS.nunu, reason: '幫使用者看清界線與可說出口的保護性回應。' }
        : riskProblem
          ? { agent: AGENTS.jingjing, reason: '把不安拆成風險確認與具體準備步驟。' }
          : { agent: AGENTS.amu, reason: '把卡住的情緒轉成一個低負擔、可以立刻開始的下一步。' };

  const selections = [
    { agent: AGENTS.awen, reason: '先讓情緒降載，避免使用者在崩潰狀態下被空泛鼓勵。' },
    { agent: AGENTS.asi, reason: '釐清真正卡住的問題與下一個可處理點。' },
    third,
  ].filter(
    (selection, index, selections) =>
      selections.findIndex((candidate) => candidate.agent.id === selection.agent.id) === index,
  );

  return {
    selections,
    summary: `這輪是情緒困境加問題處理情境，我優先邀請${selections.map((s) => s.agent.name).join('、')}，先穩住情緒，再把眼前問題拆成可執行下一步，而不是停在陪聊或空泛鼓勵。`,
  };
}
...
function buildConciseReplyInstruction(userMessage: string): string | null {
  ...
  const asksForDirectAnswer =
    /(?:請|麻煩|幫我|直接|務必)*(?:用|以)?(?:一句話|一句)(?:直接)?(?:回答|回覆|講|說|輸出)|(?:請|麻煩|幫我|直接|務必)*(?:直接)?(?:回答|回覆|講|說|輸出).*?(?:用|以)?(?:一句話|一句)|\b(?:only answer|one sentence)\b/i.test(
      text,
    );
  ...
  if (
    /(?:回答|回覆|輸出).*(?:不要加其他文字|不要其他文字|不要解釋|不要補充|不要前言)|(?:不要加其他文字|不要其他文字|不要解釋|不要補充|不要前言).*(?:回答|回覆|輸出)/i.test(
      text,
    )
  ) {
...
function extractStrictShortAnswer(text: string): string {
  ...
  const named = cleaned.match(
    /(?:答案是|答案為|名稱是|代號是|代號|名稱)\s*([A-Za-z0-9_\-\u4e00-\u9fff]+)/,
  );
...
function updateSessionTitleInBackground(params: {
  ...
  void (async () => {
    try {
      ...
      if (cleanTitle) {
        sqlite
          .prepare('UPDATE chat_sessions SET title = ? WHERE id = ? AND title = ?')
          .run(cleanTitle, sessionId, fallbackTitle);
      }
    } catch (titleErr) {
      console.error(
        '[chat-title] Background title generation failed:',
        (titleErr as Error).message,
      );
    }
  })();
}
...
router.get('/folders', (req: Request, res: Response) => {
  try {
    const folders = sqlite
      .prepare(
        'SELECT * FROM chat_folders WHERE user_id = ? ORDER BY sort_order ASC, created_at ASC',
      )
      .all(req.userId);
    res.json({ folders });
  } catch (err: any) {
...
router.post('/folders', requireAuth, (req: Request, res: Response) => {
  try {
    ...
    const result = sqlite
      .prepare('INSERT INTO chat_folders (name, icon, user_id) VALUES (?, ?, ?)')
      .run(name.trim(), icon || '💬', req.userId);
    const folder = sqlite
...
router.put('/folders/:id', requireAuth, (req: Request, res: Response) => {
  try {
    ...
    const result = sqlite
      .prepare(
        'UPDATE chat_folders SET name = COALESCE(?, name), icon = COALESCE(?, icon) WHERE id = ? AND user_id = ?',
      )
...
router.delete('/folders/:id', requireAuth, (req: Request, res: Response) => {
  try {
    ...
    sqlite
      .prepare('UPDATE chat_sessions SET folder_id = NULL WHERE folder_id = ? AND user_id = ?')
      .run(id, req.userId);
...
router.put('/sessions/:id', (req: Request, res: Response) => {
  try {
    ...
    if (folderOwnership === undefined) {
      sqlite
        .prepare('UPDATE chat_sessions SET title = COALESCE(?, title) WHERE id = ? AND user_id = ?')
        .run(title || null, id, req.userId);
    } else {
      sqlite
        .prepare(
          'UPDATE chat_sessions SET title = COALESCE(?, title), folder_id = ? WHERE id = ? AND user_id = ?',
        )
...
router.delete('/sessions/:id', (req: Request, res: Response) => {
  try {
    ...
    sqlite.prepare('DELETE FROM chat_messages WHERE session_id = ?').run(id);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const imageUrl = req.file ? `/images/chat/${path.basename(req.file.path)}` : null;
      ...
      if (isFirstMessage) {
        sqlite
          ...
          .run(fallbackSessionTitle, sessionId);
      }
      ...
      try {
        ...
        for (const r of fileResults) {
          contextParts.push(`[檔案: ${r.filename}] ${r.ai_summary || r.snippet}`);
        }
      } catch {
      ...
      const intentAgents =
        practicalIntent || directiveAdviceIntent
          ? []
          : selections.map((s) => ({
              id: s.agent.id,
              name: s.agent.name,
      ...
      const intentResult = {
        agents:
          practicalIntent || directiveAdviceIntent
            ? []
            : selections.map((s) => ({ id: s.agent.id, reason: s.reason })),
        summary: selectionSummary,
      };
      ...
      sendEvent({
        ...
        message:
          practicalIntent || directiveAdviceIntent
            ? '直接整理可用答案'
            : supportActionIntent
              ? `派出 ${selectedAgents.length} 位好友穩住情緒並拆解下一步`
              : `派出 ${selectedAgents.length} 位好友討論`,
        agents:
          practicalIntent || directiveAdviceIntent
            ? []
      ...
      const chatUserData = sqlite
        .prepare('SELECT nickname, custom_instructions FROM users WHERE id = ?')
        .get(req.userId) as { nickname: string; custom_instructions: string } | undefined;
          ...
          : await Promise.all(
              selectedAgents.map((agent) => {
                return runChatAgent(
                  ...
                  practicalIntent || supportActionIntent,
                  '',
                  sendEvent,
                  imagePart || undefined,
                  userNickname,
      ...
      try {
        aiResponse = planningIntent
          ...
          : practicalIntent &&
              isPracticalRefinementMessage(content) &&
              hasPracticalRefinementContext(historyStr)
            ...
            : practicalIntent
              ...
              : supportActionIntent
                ? await synthesizeSupportActionChat(
                    agentResults,
                    content,
                    contextStr,
                    memoryStr,
        ...
        } else if (supportActionIntent) {
          aiResponse = buildSupportActionEmergencyResponse(content, historyStr);
          sendEvent({ type: 'synthesizing', content: aiResponse });
      ...
      if (isFirstMessage) {
        updateSessionTitleInBackground({
          sessionId,
          content,
          aiResponse,
      ...
      if (aborted && userMessageId && !assistantSaved) {
        sqlite
          .prepare("DELETE FROM chat_messages WHERE id = ? AND role = 'user'")
          .run(userMessageId);
        if (req.file?.path) {
          try {
